...
public class TerminationDetailReportViewModel : BindableBase, INavigationAware
{
    ...
    #region Termination Headcount Detail Report View Properties

    public static string Title => "Termination Headcount Detail Report";
    ...
    public TerminationDetailReportViewModel(IDialogCoordinator dialogCoordinator, IErrorHandler errorHandler, IDataProcessor dataProcessor, IWorksheet worksheet)
    {
    ...
    private async void CreateReport()
    {
        ...
        try
        {
            ...
            controller.SetMessage(message);
            await Task.Run(() => Thread.Sleep(1));
            await Task.Run(() => _worksheet.CreateEEOHeadcountDetailReport(data.ToList(), StartDate, EndDate, false, _isOfficeVisible, _tokensource.Token));
