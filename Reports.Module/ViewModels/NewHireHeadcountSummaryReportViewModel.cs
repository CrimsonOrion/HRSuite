...
public class NewHireHeadcountSummaryReportViewModel : BindableBase, INavigationAware
{
    ...
    #region New Hire Headcount Summary Report View Properties

    public static string Title => "New Hire Headcount Summary Report";
    ...
    public NewHireHeadcountSummaryReportViewModel(IDialogCoordinator dialogCoordinator, IErrorHandler errorHandler, IDataProcessor dataProcessor, IWorksheet worksheet)
    {
    ...
    private async void CreateReport()
    {
        ...
        try
        {
            ...
            // Get Data
            IEnumerable<EEOHeadcountDetailReportModel> data = await _dataProcessor.GetNewHireHeadcount(StartDate, EndDate);
            ...
            controller.SetMessage(message);
            await Task.Run(() => Thread.Sleep(1));
            await Task.Run(() => _worksheet.CreateEEOHeadcountSummaryReport(data.ToList(), StartDate, EndDate, true, _isOfficeVisible, _tokensource.Token));
