...
public class EmployeeListReportViewModel : BindableBase, INavigationAware
{
    ...
    #region Employee List Report View Properties

    public static string Title => "Employee List Report";
    ...
    public EmployeeListReportViewModel(IDialogCoordinator dialogCoordinator, IErrorHandler errorHandler, IDataProcessor dataProcessor, IWorksheet worksheet)
    {
    ...
    private async void CreateReport()
    {
        ...
        try
        {
            ...
            controller.SetMessage(message);
            await Task.Run(() => Thread.Sleep(1));
            await Task.Run(() => _worksheet.CreateEmployeeListReport(data.OrderBy(_ => _.Employee.LastName).ThenBy(_ => _.Employee.FirstName).ThenBy(_ => _.Employee.Id).ToList(), _isOfficeVisible, _tokensource.Token));
