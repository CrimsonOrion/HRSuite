...
public class SafetyMeetingAttendanceListReportViewModel : BindableBase, INavigationAware
{
    ...
    #region Safety Meeting Attendance List Report View Properties

    public static string Title => "Safety Meeting Attendance List Report";
    ...
    public SafetyMeetingAttendanceListReportViewModel(IDialogCoordinator dialogCoordinator, IErrorHandler errorHandler, IDataProcessor dataProcessor, IWorksheet worksheet)
    {
    ...
    private async void CreateReport()
    {
        ...
        try
        {
            ...
            IEnumerable<EmployeeJobReportModel> data = await _dataProcessor.GetAgeListAsync();

            // Create Report
            message = "Creating report...";
            controller.SetMessage(message);
            await Task.Run(() => Thread.Sleep(1));
            await Task.Run(() => _worksheet.CreateSafetyMeetingAttenListReport(data.OrderBy(_ => _.Employee.LastName).ThenBy(_ => _.Employee.FirstName).ToList(), _isOfficeVisible, _tokensource.Token));
