...
public class CDLReportViewModel : BindableBase, INavigationAware
{
    ...
    #region CDL Report View Properties

    public static string Title => "CDL Report";
    ...
    public CDLReportViewModel(IDialogCoordinator dialogCoordinator, IErrorHandler errorHandler, IDataProcessor dataProcessor, IWorksheet worksheet)
    {
    ...
    private async void CreateReport()
    {
        ...
        try
        {
            ...
            IEnumerable<EmployeeModel> data = await _dataProcessor.GetActiveEmployeeCDLAsync();

            // Create Report
            message = "Creating report...";
            controller.SetMessage(message);
            await Task.Run(() => Thread.Sleep(1));
            await Task.Run(() => _worksheet.CreateCDLReport(data.ToList(), _isOfficeVisible, _tokensource.Token));
